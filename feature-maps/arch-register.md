...
- @id:ar Arch Register provides services to manage architecture entities, projects, documentation, and administrative
  capabilities.
    ...
    - @id:ar.entities Users can maintain a structured catalog of architectural entities and their relationships.
        ...
        - @id:ar.entities.blast-radius Users can open a Blast radius panel from an entity's drawer (a "Blast
          radius" section, present by default) or from a relation instance (a "View blast radius" action in the
          Relations browser's row menu, and a "View blast radius" button in the relation detail popover shown by
          the entity graph and Topology views). The panel runs a bounded, permission-aware traversal outward from
          the selected entity or relation's two endpoints across every visible reference, containment, and typed
          relation, grouped and ranked by lifecycle, owner, schema, and available criticality metadata, and lists
          each reachable entity with an explainable path back to the subject; clicking a result opens that entity's
          drawer. Users can filter results by depth (direct, or an extended two/three-hop traversal), owner, entity
          schema, and — for extended depths — the schemas encountered along the path. Restricted entities and
          relations are never disclosed through the traversal or its result counts, reusing the same
          permission-scoping the entity query engine and dependents lookups already apply.
