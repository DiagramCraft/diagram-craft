...
const test = baseTest.extend<{ seeded: { projectId: string; wsProjectId: string } }>({
  seeded: [
    async ({ server }, use) => {
      ...
      const projectA = await server.db.projectsFiles.createProject({
        id: 'e2e-tmpl-proj-a',
        workspace: 'default',
      ...
      const projectB = await server.db.projectsFiles.createProject({
        id: 'e2e-tmpl-proj-b',
        workspace: 'default',
      ...
      const wsTemplateFile = await server.db.projectsFiles.upsertProjectFile({
        workspace: 'default',
      ...
      await server.db.projectsFiles.updateProjectFileTemplateStatus(
        'default', projectA.id, wsTemplateFile.id, true, true, now
      );

      // Project-level template in project A
      const projATemplateFile = await server.db.projectsFiles.upsertProjectFile({
        workspace: 'default',
        project_id: projectA.id,
      ...
      const projBTemplateFile = await server.db.projectsFiles.upsertProjectFile({
        workspace: 'default',
