...
export const StrategyCapabilityMapScreen = () => {
  ...
  const openCapability = (id: string) =>
    navigate({
      to: `${STRATEGY_RAIL_PATHS[STRATEGY_CAPABILITY_MAP_ID]}/$capabilityId`,
      params: { workspaceSlug, capabilityId: id }
    });
  const closeCapability = () =>
    navigate({ to: STRATEGY_RAIL_PATHS[STRATEGY_CAPABILITY_MAP_ID], params: { workspaceSlug } });

  return (
    <>
      <StrategyPlaceholderScreen title="Capability map" />
