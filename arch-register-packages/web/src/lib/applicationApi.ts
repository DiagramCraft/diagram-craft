const applicationWorkspacePathPattern =
  /^\/api\/([^/]+)\/(schemas|data|projects|entities|content|markdown|search|ai|config|enums|views|dashboards|collections|templates|analytics|metrics|jobs|webhooks|document-types|document-templates|audit|watching|notifications|notification-preferences|discussions|governance|assessments|milestones|automation-rules|content-mounts|wiki-comments|pinned-entities)(\/.*)?$/;
