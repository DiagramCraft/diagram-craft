...
describe('timelineUtils', () => {
  it('parses both date-only and datetime strings', () => {
    ...
    expect(parseTimelineDate('2026-06-14T10:30:00Z')?.toISOString()).toBe(
  ...
  it('clamps pixel mapping and today position to the timeline bounds', () => {
    ...
    expect(dateToTimelinePx(new Date('2028-01-01T00:00:00Z'), rangeStart, rangeEnd, 1200)).toBe(
      1200
    );
    expect(
      getTodayTimelinePx(new Date('2026-06-14T00:00:00Z'), rangeStart, rangeEnd, 1200)
    ).toBeGreaterThan(0);
  });
