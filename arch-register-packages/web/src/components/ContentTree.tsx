...
import {
  buildContentFolderTree, findContentFilePath, parentPath, type ContentFolder,
  type ContentFolderNode
} from '../lib/contentPath';
import {
  deleteConfirmLabel, deleteMessage, deleteTitle, entityTypeLabel, fileMenuTargetType,
  getFileNodeIcon, type MenuTarget
...
export const ContentTree = forwardRef<ContentTreeHandle, Props>(function ContentTree({
  ...
  const isExpanded = (path: string) =>
    activePath?.startsWith(`${path}/`) || (initiallyExpanded ? !collapsed.has(path) : expanded.has(path));

  const toggle = (path: string) => {
    if (initiallyExpanded) {
      setCollapsed(previous => {
        const next = new Set(previous); next.has(path) ? next.delete(path) : next.add(path); return next;
      });
    } else {
  ...
  const renderMoveNodes = (file: ProjectFile, nodes: ContentFolderNode[], current: string | null): ReactNode =>
  ...
  const renderMenu = (target: MenuTarget) => {
    if (target.type === 'folder') return <>
      <Menu.Item leftSlot={<TbPlus size={13} />} onClick={() => { setMenu(null); onCreateDiagram(target.path); }}>New diagram</Menu.Item>
      <Menu.Item leftSlot={<TbFolderOpen size={13} />} onClick={() => { setMenu(null); onCreateFolder(target.path); }}>New folder</Menu.Item>
      <Menu.Item leftSlot={<TbUpload size={13} />} onClick={() => { setMenu(null); openUpload(target.path); }}>Upload file</Menu.Item>
      <Menu.Item leftSlot={<TbFileText size={13} />} onClick={() => { setMenu(null); onCreateMarkdown(target.path); }}>New wiki page</Menu.Item>
      <Menu.Separator />
      <Menu.Item leftSlot={<TbPencil size={13} />} onClick={() => setRenameTarget(target)}>Rename</Menu.Item>
      <Menu.Separator />
      <Menu.Item type="danger" leftSlot={<TbTrash size={13} />} onClick={() => setDeleteTarget(target)}>Delete</Menu.Item>
    </>;

    const current = parentPath(target.file.path);
    return <>
      {target.type === 'file' && <>
        <Menu.Item leftSlot={<TbDownload size={13} />} onClick={() => { setMenu(null); onDownload(target.file); }}>Download</Menu.Item>
        <Menu.Separator />
      </>}
      {target.type === 'diagram' && <>
  ...
  const fileRow = (file: ProjectFile, depth?: number) => <TreeRow
  ...
  return <>
    ...
    {menu && <ContextMenu.Imperative x={menu.x} y={menu.y} onClose={() => setMenu(null)}>{renderMenu(menu.target)}</ContextMenu.Imperative>}
    <RenameDialog open={!!renameTarget}
      currentName={renameTarget ? renameTarget.type === 'folder' ? renameTarget.path : renameTarget.file.name : ''}
    ...
    <DeleteConfirmationDialog open={!!deleteTarget}
      title={deleteTarget ? deleteTitle(deleteTarget.type) : ''}
      message={deleteTarget ? deleteMessage(deleteTarget) : ''} detail="This can't be undone."
