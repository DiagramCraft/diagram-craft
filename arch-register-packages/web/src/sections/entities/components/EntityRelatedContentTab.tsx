...
export const EntityRelatedContentTab = ({ workspaceId, entityId }: { workspaceId: string; entityId: string }) => {
  ...
  const groups = useMemo(() => {
    const grouped = new Map<string, { typeName: string; typeColor: string | null; typeIcon: string | null; fieldName: string; items: typeof data }>();
    for (const item of data) {
      const typeName = item.document_type_name ?? 'Untyped Markdown';
      const key = `${typeName} · ${item.field_name}`;
      const existing = grouped.get(key);
      if (existing) existing.items.push(item);
      else grouped.set(key, { typeName, typeColor: item.document_type_color, typeIcon: item.document_type_icon, fieldName: item.field_name, items: [item] });
    }
  ...
  return (
    <div style={{ padding: 20, display: 'grid', gap: 18 }}>
      {groups.map(([key, group]) => (
        <section key={key}>
          <h3 style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, margin: '0 0 8px' }}>
            <TypeBadge color={group.typeColor ?? 'var(--base-fg-dim)'} name={group.typeName} icon={group.typeIcon} size={18} />
          ...
          <div style={{ display: 'grid', gap: 2 }}>
            {group.items.map(item => (
              <TreeRow
                ...
                trailing={<span className="dim" style={{ fontSize: 10 }}>{item.file.path}</span>}
                onClick={() => navigate(
                  item.scope === 'project'
