...
import { contentDownloadUrl, useContentScopeOperations, useContentTree, type ContentScope } from '../../hooks/useContentScope';
...
export const EntityContentSidebar = ({ workspaceSlug, entityId }: { workspaceSlug: string; entityId: string }) => {
  ...
  const openFile = (file: ProjectFile) => {
    ...
    if (projectId) navigate(file.type === 'markdown'
    ...
    else navigate(file.type === 'markdown'
  ...
  return <>
    ...
    <AddDiagramDialog open={diagramFolder !== undefined} onClose={() => setDiagramFolder(undefined)}
      onCreated={file => { setDiagramFolder(undefined); navigate(entityDiagramRoute(workspaceSlug, asEntityPublicId(entityId), file.id)); }}
      workspaceId={workspaceSlug} context="entity" entityId={entityId} folder={diagramFolder ?? null} />
    <AddMarkdownDialog open={markdownFolder !== undefined} onClose={() => setMarkdownFolder(undefined)}
      onCreated={file => { setMarkdownFolder(undefined); navigate(entityMarkdownRoute(workspaceSlug, asEntityPublicId(entityId), file.id, { mode: 'edit' })); }}
