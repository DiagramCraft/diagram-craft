...
const entity = {
  _schema: { id: 'service' }, _name: 'Live name', _slug: 'service', _namespace: '',
  _description: 'Live description', _owner: { id: 'team-live' }, _lifecycle: null,
  _targetLifecycle: null, _targetLifecycleDate: null, _tags: ['live'], _links: [],
  _visibilityMode: null, criticality: 'high'
} as never;
const base = { name: 'Base name', description: 'Base description', owner: 'team-base', tags: ['base'], data: { criticality: 'low' } };
const proposed = { name: 'Planned name', description: 'Base description', owner: 'team-base', tags: ['planned'], data: { criticality: 'medium' } };

describe('project snapshot state', () => {
  it('reports only fields changed in both the plan and live entity', () => {
    expect(findSnapshotConflicts(entity, schema, proposed, base).map(conflict => conflict.key)).toEqual([
      'name', 'tags', 'data.criticality'
    ]);
  });

  it('resolves conflicts using the explicit current choice', () => {
    const resolved = resolveSnapshotEntityData({
      entity, schema, proposed, base,
