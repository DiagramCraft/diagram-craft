...
export const BaselineListScreen = () => {
  ...
  const createError =
    createBaseline.error instanceof ApiError
      ...
      : createBaseline.error
        ...
        : null;
  const loadError =
    error instanceof ApiError ? error.message : error ? 'Could not load baselines' : null;
  ...
  const resolveScope = (): CreateBaselineRequest['scope'] | null => {
    ...
    return entityIds.length > 0 && entityIds.length <= 1000
    ...
    <main className={styles.screen}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            Named, immutable catalog snapshots for governance and comparison.
          </p>
        </div>
      </header>
      ...
      <section className={styles.panel}>
        <div>
          ...
          <p className={styles.subtitle}>
            Capture the visible workspace catalog at a selected point in time.
          </p>
        </div>
        <form className={styles.form} onSubmit={submit}>
          ...
          {scopeKind === 'project' && (
            <label className={styles.field}>
              Project contents
              <select
                value={scopeProjectScope}
                onChange={event => setScopeProjectScope(event.target.value as 'project' | 'all')}
              >
          ...
          {scopeKind === 'selection' && (
            <label className={`${styles.field} ${styles.description}`}>
              Entity IDs
              <textarea
                value={scopeEntityIds}
                onChange={event => setScopeEntityIds(event.target.value)}
                rows={1}
          ...
          <label className={styles.checkbox}>
            <input
              type="checkbox"
              checked={includePlannedChanges}
              onChange={event => setIncludePlannedChanges(event.target.checked)}
            />
          ...
          <label className={styles.checkbox}>
            <input
              type="checkbox"
              checked={includeOverdueChanges}
              onChange={event => setIncludeOverdueChanges(event.target.checked)}
            />
      ...
      <section className={styles.panel}>
        ...
        {isLoading ? (
          <p className={styles.subtitle}>Loading baselines…</p>
        ) : baselines.length === 0 ? (
                  ...
                  <td>
                    <button
                      ...
                      onClick={() =>
                        navigate({
                          to: '/$workspaceSlug/baselines/$baselineId',
                          params: { workspaceSlug, baselineId: baseline.id }
                        })
                      }
                    >
                      {baseline.name}
