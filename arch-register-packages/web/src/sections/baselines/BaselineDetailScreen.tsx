...
export const BaselineDetailScreen = () => {
  ...
  const remove = async () => {
    if (
      !window.confirm(`Delete baseline “${baseline.name}”? Historical evidence will be retained.`)
    )
      return;
    ...
    <main className={styles.screen}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            {baseline.description ?? 'Read-only architecture baseline'}
          </p>
        </div>
      ...
      <section className={styles.panel}>
        <dl className={styles.metadata}>
          <div>
            <dt>Status</dt>
            <dd>
              <span className={styles.status}>{baseline.status}</span>
            </dd>
          </div>
          <div>
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {comparison.added.map(entity => (
                  <tr key={`added-${entity._uid}`}>
                    <td>{entity._name}</td>
                    <td>Added</td>
                  </tr>
                ))}
                {comparison.removed.map(entity => (
                  <tr key={`removed-${entity._uid}`}>
                    <td>{entity._name}</td>
                    <td>Removed</td>
                  </tr>
                ))}
                {comparison.changed.map(entry => {
                  ...
                  return (
                    <tr key={`changed-${entry.entity._uid}`}>
                      <td>{entry.entity._name}</td>
                      <td>{changedFields.length > 0 ? changedFields : 'Restricted changes'}</td>
                    </tr>
                  );
                })}
              </tbody>
            ...
            <tbody>
              {baseline.entities.map(entity => (
                <tr key={entity._uid}>
                  ...
                  <td>{entity._owner?.name ?? '—'}</td>
                  <td>{entity._lifecycle?.name ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {baseline.relations.map(relation => (
                <tr key={relation._uid}>
                  <td>{relation._in.name}</td>
                  <td>{relation._schema.name}</td>
                  <td>{relation._out.name}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className={styles.panel}>
        <div>
          ...
          <p className={styles.subtitle}>
            Connect this immutable snapshot to the project, change, document, milestone, or
            governance case that explains it.
          </p>
        </div>
        <form className={styles.toolbar} onSubmit={addReference}>
          <select
            value={targetType}
            onChange={event => setTargetType(event.target.value as BaselineLinkTargetType)}
          >
          ...
          <button
            className={`${styles.button} ${styles.primary}`}
            type="submit"
            disabled={createLink.isPending || targetId.trim().length === 0}
          >
            {createLink.isPending ? 'Adding…' : 'Add reference'}
          </button>
        </form>
        {baseline.links.length === 0 ? (
          <p className={styles.subtitle}>No references have been added.</p>
        ) : (
          <table className={styles.table}>
            <thead>
              <tr>
