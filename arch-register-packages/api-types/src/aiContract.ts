...
const aiConfigSchema = z.object({
  ...
  temperature: z.number().nullable().describe('Temperature parameter for generation (0-2, null for default)'),
...
const aiConfigUpdateSchema = z.object({
  ...
  temperature: z.number().min(0).max(2).nullable().optional().describe('Temperature parameter (0-2, null for default)'),
  system_prompt: z.string().nullable().optional().describe('Custom system prompt (null for default)'),
  enabled: z.boolean().optional().describe('Enable or disable AI features')
});

// ── Contract ──────────────────────────────────────────────────

export const aiContract = oc
  .tag('AI')
  .router({
    ai: {
      ...
      createConversation: oc
        ...
        .input(
          z.object({
            params: ws,
            body: z.object({ title: z.string().optional().describe('Conversation title (auto-generated if not provided)') })
          })
        )
      ...
      deleteConversation: oc
        .route({
          ...
          description: 'Deletes an AI conversation and all its messages. This operation cannot be undone.',
          tags: ['AI']
        })
      ...
      getConfig: oc
        .route({
          ...
          description: 'Retrieves the AI configuration for the workspace, including provider settings and enabled status.',
          tags: ['AI']
        })
        .input(z.object({ params: ws }))
      ...
      updateConfig: oc
        .route({
          ...
          description: 'Updates the AI configuration for the workspace, including provider, API key, model, and other settings.',
          tags: ['AI']
        })
        .input(
          z.object({
      ...
      extract: oc
        .route({
          ...
          description: 'Uses AI to extract structured entity data from unstructured text. Returns a list of potential entities.',
          tags: ['AI']
        })
        .input(
          z.object({
      ...
      chat: oc
        .route({
          ...
          description: 'Sends a message to the AI and receives a streaming response. Supports conversation context and custom state.',
          tags: ['AI']
        })
        .input(
          z.object({
            ...
            body: z.object({
              ...
              forwardedProps: z.record(z.string(), z.unknown()).optional().describe('Additional properties'),
