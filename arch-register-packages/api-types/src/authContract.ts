...
const authMeResponseSchema = z.object({
  ...
  team_assignments_by_workspace: z.record(z.string(), z.array(teamAssignmentSchema)).describe('Team assignments organized by workspace'),
  workspace_roles: z.record(z.string(), z.string()).describe('Workspace role assignments (workspace ID to role ID)'),
  ...
  teams_by_workspace: z.record(z.string(), z.array(teamSchema)).describe('Teams organized by workspace')
});
...
export const authPublicContract = oc
  .tag('Auth')
  .router({
    auth: {
      config: oc
        .route({
          ...
          description: 'Retrieves the authentication mode configuration (local or OIDC). Used by clients to determine which authentication flow to use.',
          tags: ['Auth']
        })
      ...
      login: oc
        .route({
          ...
          description: 'Authenticates a user with username and password (local authentication only). Returns JWT tokens for subsequent API requests.',
          tags: ['Auth']
        })
        .input(z.object({
          body: z.object({
      ...
      oidcAuthorize: oc
        .route({
          ...
          description: 'Retrieves the OIDC provider authorization URL for initiating the OAuth2 flow. Client should redirect user to this URL.',
          tags: ['Auth']
        })
        .output(z.object({
          query: z.object({
      ...
      refresh: oc
        .route({
          ...
          description: 'Obtains a new access token using a refresh token. Used when the current access token has expired.',
          tags: ['Auth']
        })
        .input(z.object({
          body: z.object({
            refresh_token: z.string().optional().describe('Refresh token (can also be provided via cookie)')
          }).optional()
        }))
        .output(tokenPairSchema),
      logout: oc
...
export const authProtectedContract = oc
  .tag('Auth')
  .router({
    authProtected: {
      me: oc
        .route({
          ...
          description: 'Retrieves the authenticated user\'s profile, including roles, permissions, team assignments, and workspace memberships.',
          tags: ['Auth']
        })
      ...
      updateUser: oc
        .route({
          ...
          description: 'Updates user profile information such as display name and color preference. Users can only update their own profile unless they have admin permissions.',
          tags: ['Auth']
        })
        .input(
          z.object({
      ...
      getGlobalRoles: oc
        .route({
          ...
          description: 'Retrieves the global role assignments for a specific user. Requires admin permissions.',
          tags: ['Auth']
        })
        .input(z.object({ params: z.object({ id: z.string().regex(UUID_REGEX).describe('User identifier (UUID)') }) }))
      ...
      replaceGlobalRoles: oc
        .route({
          ...
          description: 'Replaces all global role assignments for a user. This is a full replacement operation. Requires global admin permissions.',
          tags: ['Auth']
        })
        .input(
          z.object({
