...
const allTemplatesResponseSchema = z.object({
  workspaceTemplates: z.array(projectFileSchema).describe('Templates available across the entire workspace'),
  projectTemplates: z.record(z.string(), z.array(projectFileSchema)).describe('Templates organized by project ID')
});

// Response for GET /{workspace}/projects/{projectId}/templates (flat lists)
const projectTemplatesResponseSchema = z.object({
  workspaceTemplates: z.array(projectFileSchema).describe('Workspace-level templates available to this project'),
  projectTemplates: z.array(projectFileSchema).describe('Project-specific templates')
});

// ── Contract ──────────────────────────────────────────────────

export const workspaceTemplateContract = oc
  .tag('Templates')
  .router({
    templates: {
      listAll: oc
        .route({
          ...
          description: 'Retrieves all diagram and file templates in the workspace, organized by project. Includes both workspace-level and project-specific templates.',
          tags: ['Templates']
        })
      ...
      listForProject: oc
        .route({
          ...
          description: 'Retrieves templates available for a specific project, including both workspace-level templates and project-specific templates.',
          tags: ['Templates']
        })
      ...
      toggleStatus: oc
        .route({
          ...
          description: 'Marks a file as a template or removes its template status. Can set as project-level or workspace-level template.',
          tags: ['Templates']
        })
        .input(
          z.object({
            ...
            body: z.object({
              is_template: z.boolean().describe('Whether the file should be marked as a project template'),
              is_workspace_template: z.boolean().describe('Whether the file should be marked as a workspace-level template')
            })
          })
        )
      ...
      createFromTemplate: oc
        .route({
          ...
          description: 'Creates a new file in the project by copying from an existing template. The template can be from the same project or a workspace-level template.',
          tags: ['Templates']
        })
        .input(
          z.object({
            ...
            body: z.object({
              ...
              folder: z.string().nullable().optional().describe('Optional folder path where the new file should be created')
            })
          })
        )
        .output(projectFileSchema)
