...
const MAX_TOTAL_FILE_BYTES = positiveEnvNumber('EXTERNAL_CONTENT_MAX_TOTAL_FILE_BYTES', 500 * 1024 * 1024);
const MAX_REPOSITORY_BYTES = positiveEnvNumber('EXTERNAL_CONTENT_MAX_REPOSITORY_BYTES', 1 * 1024 * 1024 * 1024);
const MAX_CONCURRENT_FILE_READS = Math.max(1, Math.floor(positiveEnvNumber('EXTERNAL_CONTENT_MAX_CONCURRENT_FILE_READS', 8)));
...
const ensureMirror = async (sourceId: string, config: GitSourceConfig) => {
  ...
  const repositoryBytes = objectStats.stdout.toString('utf8').split('\n').reduce((total, line) => {
...
const parseTree = (output: string, sourcePath: string) => {
  ...
  const entries = output.split('\0').filter(Boolean).map(record => {
    ...
    if (!Number.isFinite(size) || size > MAX_FILE_BYTES) throw new Error(`Git file '${relative}' exceeds the ${MAX_FILE_BYTES} byte limit`);
  ...
  if (entries.length > MAX_FILES) throw new Error(`Git source contains more than the ${MAX_FILES} file limit`);
  const totalBytes = entries.reduce((total, entry) => total + entry.size, 0);
  if (totalBytes > MAX_TOTAL_FILE_BYTES) throw new Error(`Git source exceeds the ${MAX_TOTAL_FILE_BYTES} byte content limit`);
  return entries;
};
...
export const readGitSnapshot = async (repoPath: string, sourcePath: string): Promise<GitSnapshot> => {
  ...
  await Promise.all(Array.from({ length: Math.min(MAX_CONCURRENT_FILE_READS, entries.length) }, () => readNext()));
