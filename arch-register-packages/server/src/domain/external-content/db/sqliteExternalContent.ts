...
export class SqliteExternalContentDatabase
  ...
  implements ExternalContentDatabase
{
  async createSource(input: ExternalContentSourceDbCreate) {
    this.run(
      ...
      [
        ...
        input.enabled ? 1 : 0,
        input.status,
        iso(input.last_attempt_at),
        iso(input.last_synced_at),
        input.last_revision,
        input.last_error,
  ...
  async getSource(workspace: string, id: string) {
    return this.get(
      'SELECT * FROM external_content_source WHERE workspace = ? AND id = ?',
      [workspace, id],
  ...
  async listSources(workspace: string) {
    return this.all(
      'SELECT * FROM external_content_source WHERE workspace = ? ORDER BY created_at, id',
      [workspace],
      externalContentMappers.source
    );
  }
  ...
  private async getById(id: string) {
    return this.get(
      'SELECT * FROM external_content_source WHERE id = ?',
      [id],
      externalContentMappers.source
    );
  }
  ...
  async getMount(workspace: string, id: string) {
    return this.get(
      'SELECT * FROM content_mount WHERE workspace = ? AND id = ?',
      [workspace, id],
      externalContentMappers.mount
    );
  }

  async listMounts(workspace: string) {
    return this.all(
      'SELECT * FROM content_mount WHERE workspace = ? ORDER BY destination_path, id',
      [workspace],
      externalContentMappers.mount
    );
  }
  ...
  async listMountsBySource(workspace: string, sourceId: string) {
    return this.all(
      'SELECT * FROM content_mount WHERE workspace = ? AND source_id = ? ORDER BY destination_path, id',
      [workspace, sourceId],
      externalContentMappers.mount
    );
  }
