...
import {
  ...
  type EntityQueryPermissionPlan
...
const addParam = (state: CompileState, value: unknown): string => {
  ...
  return state.dialectAdapter.placeholder(state.params.length);
};
...
const resolveColumn = (
  ...
  return {
    col: dialectAdapter.jsonFieldText(`${alias}.data`, fieldId),
    kind: 'scalar'
  };
};
...
const assessmentFieldColumn = (
  ...
  return createEntityQueryDialectAdapter(dialect).jsonFieldText(
...
const projectionRawValueRelation = (
  ...
  assertValidFieldId(fieldId);
  return createEntityQueryDialectAdapter(dialect).jsonFieldValue(`${alias}.data`, fieldId);
};
...
const projectionRawValueRelationData = (
  ...
  assertValidFieldId(fieldId);
  return createEntityQueryDialectAdapter(dialect).jsonFieldValue(dataExpression, fieldId);
};
...
const buildEntityQueryPlanInputs = (
  query: EntityQuery,
  schemas: SchemaCatalog,
  relationSchemas: RelationSchemaCatalog,
...
const buildQueryFragments = (
  ...
  const rootKind = semanticPlan.rootKind;

  const state: CompileState = {
    ...permissionPlan,
    dialect,
    dialectAdapter: createEntityQueryDialectAdapter(dialect),
    workspace,
    relationSchemas,
    rootKind,
    params: [],
...
type EntityQueryFragments = ReturnType<typeof buildQueryFragments>;

const renderEntityQueryRows = (fragments: EntityQueryFragments): CompiledEntityQuery => {
