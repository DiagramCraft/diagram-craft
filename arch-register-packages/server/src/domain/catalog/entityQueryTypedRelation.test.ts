...
describe('typed scalar relation query compilation', () => {
  it('compiles a direction-aware relation hop and same-relation scalar filter', () => {
    ...
    expect(compiled.sql).toContain('JOIN relation pb_rel_query_path_0_1');
    expect(compiled.sql).toContain('pb_rel_query_path_0_1.out_entity_id = pb_root_query_path_0.id');
