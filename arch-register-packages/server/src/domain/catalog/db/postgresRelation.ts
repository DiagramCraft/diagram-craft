  ...
  async createRelationSchemaVersion(input: RelationSchemaVersionDbCreate) {
    const [row] = (await this.sql`
      INSERT INTO relation_schema_version
        (id, workspace, schema_id, version, name, category, description, in_schema_ids, out_schema_ids, in_label, out_label, fields, groups, validation_rules, color, icon, change_summary, created_by, created_at)
      VALUES
        (${input.id}, ${input.workspace}, ${input.schema_id}, ${input.version}, ${input.name}, ${input.category ?? null}, ${input.description}, ${this.json(input.in_schema_ids)}, ${this.json(input.out_schema_ids)}, ${input.in_label ?? null}, ${input.out_label ?? null}, ${this.json(input.fields)}, ${this.json(input.groups)}, ${this.json(input.validation_rules ?? [])}, ${input.color}, ${input.icon}, ${this.json(input.change_summary)}, ${input.created_by}, ${input.created_at})
      RETURNING *
    `) as DatabaseRow[];
