...
export class SqliteCatalogDatabase extends SqliteDatabaseBase implements CatalogDatabase {
  ...
  async createSchemaVersion(input: SchemaVersionDbCreate) {
    this.run(
      'INSERT INTO entity_schema_version (id, workspace, schema_id, version, name, description, fields, templates, groups, shared_field_group_links, color, icon, change_summary, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        input.id,
        input.workspace,
        input.schema_id,
