...
const runWorkspaceConfigOperation = <Result>(
  db: DatabaseAdapter,
  workspace: string,
...
export const listLifecycleStates = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
...
export const listProjectEntityTypes = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    requireWorkspaceCapability(authCtx, 'ws.view');
    return await db.workspace.listProjectEntityTypes(ws);
  });
};
...
export const listAssessmentTypes = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    requireWorkspaceCapability(authCtx, 'ws.view');
    return await db.workspace.listAssessmentTypes(ws);
  });
};
...
export const replaceAssessmentTypes = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    return await db.workspace.replaceAssessmentTypes(
      ws,
      normalized.map(type => ({ ...type, workspace: ws }))
    );
  });
};
...
export const listSupportedCurrencies = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    requireWorkspaceCapability(authCtx, 'ws.view');
    return await db.workspace.getSupportedCurrencies(ws);
  });
};
...
export const replaceSupportedCurrencies = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    return await db.workspace.replaceSupportedCurrencies(ws, normalized, normalizedDefault);
  });
};
...
export const listTeams = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    requireWorkspaceCapability(authCtx, 'ws.view');
    return await db.workspace.listTeams(ws, options);
  });
};
...
export const listTeamAssignments = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    requireWorkspaceCapability(authCtx, 'people.teams');
    return await db.workspace.listTeamAssignments(ws);
  });
};
...
export const replaceTeamAssignments = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    const owners = new Set((await db.workspace.listTeams(ws)).map(o => o.id));
    ...
    return await db.workspace.replaceTeamAssignments(ws, rows);
  });
};
...
export const listRoles = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    return resolveWorkspaceRoleDefinitions(await db.workspace.listCustomWorkspaceRoles(ws));
  });
};
...
export const updateRole = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    const parsed = parseRoleInput(input);
    const updated = await db.workspace.updateCustomWorkspaceRole(ws, roleId, {
      name: parsed.name,
      description: parsed.description,
      tone: parsed.tone,
      builtin: false,
...
export const deleteRole = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    const memberCount = await db.workspace.countWorkspaceMembersByRole(ws, roleId);
    httpAssert.true(memberCount === 0, {
      status: 409,
      message: 'Role is still assigned to workspace members'
    });

    const deleted = await db.workspace.deleteCustomWorkspaceRole(ws, roleId);
...
export const listMembers = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    const members = await db.workspace.listWorkspaceMembers(ws);
...
export const updateMemberRole = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    const [user, customRoles] = await Promise.all([
      ...
      db.workspace.listCustomWorkspaceRoles(ws)
    ]);
    ...
    return await db.workspace.setWorkspaceMemberRole(ws, userId, role, new Date());
  });
};
...
export const removeMember = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (ws, authCtx) => {
    ...
    const removed = await db.workspace.removeWorkspaceMember(ws, userId);
...
export const listUsers = async (
  ...
  return runWorkspaceConfigOperation(db, workspace, event, async (_ws, authCtx) => {
    requireWorkspaceCapability(authCtx, 'people.invite');
