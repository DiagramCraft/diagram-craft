...
CREATE TABLE entity_change_case_revision (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  case_id            UUID NOT NULL REFERENCES entity_change_case(id) ON DELETE CASCADE,
  workspace          UUID NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
  revision_number    INTEGER NOT NULL,
  policy_version     TEXT,
  resolved_policy    JSONB NOT NULL DEFAULT '{}',
  message            TEXT,
...
CREATE TABLE entity_version (
  ...
  applied_case_revision_id UUID REFERENCES entity_change_case_revision(id) ON DELETE RESTRICT,
...
CREATE TABLE entity_change_case_entity_version (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  revision_id        UUID NOT NULL REFERENCES entity_change_case_revision(id) ON DELETE CASCADE,
  workspace          UUID NOT NULL REFERENCES workspace(id) ON DELETE CASCADE,
  entity_id          UUID NOT NULL,
  ...
  diff               JSONB NOT NULL,
  applied_version_id UUID REFERENCES entity_version(id) ON DELETE RESTRICT,
  FOREIGN KEY (workspace, entity_id) REFERENCES entity(workspace, id) ON DELETE CASCADE,
