...
| `RiskDrawer` | Risk | Public id eyebrow; category/status badges; **Open record in Entities** footer action | Risk profile and residual band; attributes; aggregate coverage; mitigating controls with coverage/effectiveness; affected entities | Workspace-wide `drawer` search param from the Risks register, with legacy `/risk-compliance/risks/$riskId` links redirected; in-situ opening from controls and overview surfaces |
