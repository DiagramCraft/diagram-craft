...
const StyleContextMenu = <T extends StyleCombination | TextStyleCombination>({
  ...
  <ContextMenu.Menu>
    ...
    <Menu.Item disabled={style.differences.length === 0} onClick={() => onCreateStylesheet(style)}>
      Create Stylesheet
    </Menu.Item>
                      ...
                      {group.styles.map((textStyle, idx) => {
                        ...
                        return (
                          <ContextMenu.Root key={key}>
                            <ContextMenu.Trigger
                              element={
                                <div key={key} onClick={() => onStyleClick(textStyle)}>
                                  <div>
                                    <TbLetterCase size={18} />
                                  </div>
                                  <div>
                                    <div style={fontStyle}>{textStyle.props.text.font}</div>
                                    <div>
                                      {metaParts.join(', ')}
                                      <div>
                                        {textStyle.elements.length} element(s)
                                        {textStyle.differences.length > 0 && <span>*</span>}
                                      </div>
                                    </div>
                                  </div>
                                </div>
