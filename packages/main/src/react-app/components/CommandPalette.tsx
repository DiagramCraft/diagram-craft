...
import { ActionName, formatKeyBinding, findKeyBindingsForAction } from '@diagram-craft/canvas/keyMap';
...
export const CommandPalette = ({ open, onClose }: CommandPaletteProps) => {
  ...
  const executeCommand = useCallback((commandId: ActionName) => {
          ...
          {filteredCommands.length === 0 ? (
            ...
            filteredCommands.map((command, index) => (
              <div
                ...
                } ${!command.isEnabled ? styles.disabled : ''}`}
              ...
              >
                ...
                {command.keyBinding && (
                  <div className={styles.keyBinding}>{command.keyBinding}</div>
                )}
              </div>
            ))
          )}
