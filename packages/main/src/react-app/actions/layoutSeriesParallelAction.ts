...
declare global {
  namespace DiagramCraft {
    interface ActionMapExtensions extends ReturnType<typeof layoutSeriesParallelActions> {}
  }
}

export type LayoutSeriesParallelActionDirection = 'down' | 'up' | 'left' | 'right';

export type LayoutSeriesParallelActionArgs = {
  gap: number;
  direction: LayoutSeriesParallelActionDirection;
};

export const layoutSeriesParallelActions = (context: Application) => ({
  LAYOUT_SERIES_PARALLEL: new LayoutSeriesParallelAction(context)
});
...
export class LayoutSeriesParallelAction extends AbstractSelectionAction<Application> {
  ...
  private applyChanges(d: LayoutSeriesParallelActionArgs) {
    ...
    // Get the series-parallel layout positions
    const positions = layoutSeriesParallel(
      graph,
    ...
    UnitOfWork.executeWithUndo(diagram, 'Layout series-parallel', uow => {
