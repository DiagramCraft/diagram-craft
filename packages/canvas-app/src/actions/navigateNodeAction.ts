...
export class NavigateNodeAction extends AbstractSelectionAction {
  ...
  execute(): void {
    const selectedNodes = this.context.model.activeDiagram.selectionState.nodes;
    if (selectedNodes.length === 0) return;

    const currentNode = selectedNodes.at(-1)!
    const allNodes = Array.from(this.context.model.activeDiagram.allElements())
      .filter(element => 
        isNode(element) && 
        element !== currentNode && 
    ...
    if (this.extendSelection) {
      this.context.model.activeDiagram.selectionState.toggle(closestNode);
    } else {
      this.context.model.activeDiagram.selectionState.setElements([closestNode]);
    }
    
    this.emit('actionTriggered', {});
  }

  private findClosestNodeInDirection(currentNode: DiagramNode, candidates: DiagramNode[], direction: Direction): DiagramNode | null {
