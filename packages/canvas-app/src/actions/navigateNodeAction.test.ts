...
describe('NavigateNodeAction', () => {
  ...
  test('should extend selection when extendSelection is true', () => {
    ...
    expect(diagram.selectionState.nodes.map(n => n.id)).toContain('node1');
    expect(diagram.selectionState.nodes.map(n => n.id)).toContain('node2');
  });
  ...
  test('should navigate from the most recently selected node when multiple are selected', () => {
    ...
    expect(diagram.selectionState.nodes).toHaveLength(2);
    expect(diagram.selectionState.nodes.at(-1)!.id).toBe('node2');
