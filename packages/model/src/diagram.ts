...
import { createUndoManager, type UndoManager } from './undoManager';
...
export class Diagram extends EventEmitter<DiagramEvents> implements AttachmentConsumer, Releasable {
  ...
  *allElements(): Generator<DiagramElement> {
  ...
  unregister(element: DiagramElement) {
    if (isNode(element)) this.nodeLookup.delete(element.id);
    else if (isEdge(element)) this.edgeLookup.delete(element.id);
  }
