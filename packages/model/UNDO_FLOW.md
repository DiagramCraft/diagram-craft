...
- In the default case, structural undo is built after the mutation by `UnitOfWork.asUndoableAction(...)`.
...
5. `capture.commit()` asks for `UnitOfWork.asUndoableAction(label)`.
...
sequenceDiagram
    ...
    Capture->>Work: commit()
    Capture->>Work: asUndoableAction(label)
...
6. `capture.commit()` asks for `UnitOfWork.asUndoableAction(label)`.
